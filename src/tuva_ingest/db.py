...
that other tuva_ingest modules -- and their unit tests -- can be
...
`tuva_ingest.identifiers` for why the distinction matters). Both
...
def substitute_psql_vars(sql_text: str, variables: dict[str, str]) -> str:
    ...
    migrations/ (written psql-`-f`/`-v`-style) through a plain psycopg
    connection, without shelling out to the `psql` binary.
