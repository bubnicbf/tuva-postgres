"""`tuva-ingest healthcheck`: a single, fast, no-secrets status check
suitable for a container health probe or a human running it by hand.

Checks, in order (each can independently fail the check):
  1. PostgreSQL connectivity (can we open a connection at all).
  2. Migration state: unhealthy if any migration's checksum has drifted
     or anything is pending.
  3. Freshness: the most recent *successful* ingestion run finished
     within PIPELINE_MAX_SUCCESS_AGE_HOURS.
...
def run_healthcheck(config, *, connect_fn=connect, migrations_mod=migrations, state_mod=state) -> HealthCheckResult:
    ...
    try:
        ...
        freshness_ok = True
        try:
            last_success = state_mod.latest_successful_run(conn, config.ops_schema)
            if last_success is None:
                freshness_ok = False
                freshness_detail = "no successful run recorded"
            else:
                _run_id, _snapshot_id, finished_at, *_rest = last_success
...
def main(argv: list[str] | None = None) -> int:
    from .config import REQUIRE_DB, IngestConfig
